...
class FigurateServerGuideResource extends Resource
{
    public function handle(Request $request): Response
    {
        return Response::text(implode("\n", [
            '# Figurate MCP Guide',
            '',
            'Use this server to inspect and operate on channel-based work in Figurate.',
            '',
            'Core objects:',
            '- Channel: long-lived context and entrypoint.',
            '- Thread: active work session inside a channel.',
            '- Post: durable artifact or domain event attached to a channel or thread.',
            '- Thread Actor: human or named agent participating in a thread.',
            '',
            'Recommended workflow:',
            '1. Start with `list_channels` or `read_channel`.',
