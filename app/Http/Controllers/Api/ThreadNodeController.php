...
class ThreadNodeController extends Controller
{
    public function __construct(protected GraphNodeService $graphNodes) {}

    public function __invoke(ListGraphNodesRequest $request, string $thread): JsonResponse
    {
        /** @var User $actor */
        $actor = $request->user();
        $parent = $this->graphNodes->resolve($actor, 'thread', $thread);
        abort_unless($parent instanceof Thread, 404);
